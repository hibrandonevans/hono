 ...
 * @param {Function | false} [options.onCacheNotAvailable] - A callback function to handle when Cache API is not available. If set to `false`, logging is disabled. By default, a message is logged using `console.log`.
...
export const cache = (options: {
  ...
  if (!globalThis.caches) {
    if (options.onCacheNotAvailable === false) {
      // Do nothing - user disabled logging
    } else if (typeof options.onCacheNotAvailable === 'function') {
